...
export const CreateOrder = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if(customer){
        ...
        if(profile)
        {
                ...
                if (vendorId) {
                    ...
                    if (vendorId !== undefined) {
                        try {
                            ...
                            return res.status(500).json({ msg: 'Error while Creating Order. Failed to assign order for delivery' });
                        }
                    }
                }
            ...
            // Handle the case where vendorId is undefined
            console.error('VendorId is undefined');
            return res.status(400).json({ msg: 'Error while Creating Order. VendorId is undefined' });
        }
        }
    }
